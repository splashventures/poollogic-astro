import { Metadata } from 'next';
import Link from 'next/link';
import Image from 'next/image';
import { notFound } from 'next/navigation';
import { posts, getPostBySlug, getRelatedPosts } from '@/data/posts';
import { company } from '@/data/company';
import { Breadcrumbs } from '@/components/layout/Breadcrumbs';
import { DualCTA } from '@/components/ui/DualCTA';
import { generatePostMeta } from '@/lib/seo/generatePostMeta';
import { generateArticleSchema } from '@/lib/schema/article';
import { generateSimpleFAQSchema } from '@/lib/schema/faq';

const FALLBACK_IMAGE = '/images/pool/hero-pool.webp';
const FALLBACK_IMAGE_ALT = 'PoolLogic professional pool service in San Diego';

interface PageProps {
  params: Promise<{ slug: string }>;
}

export async function generateStaticParams() {
  return posts.map((post) => ({ slug: post.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const post = getPostBySlug(slug);
  if (!post) return {};
  return generatePostMeta(post);
}

export default async function BlogPostPage({ params }: PageProps) {
  const { slug } = await params;
  const post = getPostBySlug(slug);
  if (!post) notFound();

  const relatedPosts = getRelatedPosts(post);
  const articleSchema = generateArticleSchema(post);
  const faqSchema = generateSimpleFAQSchema(post.faqs);

  const categoryPath = `/resources/category/${post.category}/`;
  const categoryLabel = post.categoryName;

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(articleSchema) }}
      />
      {faqSchema && (
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }}
        />
      )}

      {/* Breadcrumbs */}
      <div className="py-2">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumbs
            items={[
              { name: 'Home', url: '/' },
              { name: 'Resources', url: '/resources/' },
              { name: post.title, url: `/resources/${post.slug}/` },
            ]}
          />
        </div>
      </div>

      {/* Article Header */}
      <header className="bg-gradient-to-br from-[#00568c] to-[#00568c]/80 text-white py-12 lg:py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link
            href={categoryPath}
            className="inline-flex items-center gap-1 text-sm font-medium text-[#88d2c7] hover:text-[#88d2c7]/80 mb-4"
          >
            {categoryLabel}
          </Link>
          <h1 className="text-3xl lg:text-4xl font-bold mb-4">{post.title}</h1>
          <div className="flex flex-wrap items-center gap-4 text-blue-200 text-sm">
            <Link href="/about/kyle-bowman/" className="inline-flex items-center gap-2 hover:text-white transition-colors">
              <Image
                src="/images/pool/kyle-bowman.webp"
                alt={post.author}
                width={28}
                height={28}
                className="rounded-full object-cover"
              />
              By {post.author}
            </Link>
            <span className="hidden sm:inline">&middot;</span>
            <time dateTime={post.date}>
              {new Date(post.date).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
              })}
            </time>
            <span className="hidden sm:inline">&middot;</span>
            <span>{post.readTime}</span>
          </div>
        </div>
      </header>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
          {/* Table of Contents - Sidebar */}
          {post.headings.length > 0 && (
            <aside className="hidden lg:block lg:col-span-1">
              <div className="sticky top-32">
                <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">
                  Table of Contents
                </h2>
                <nav>
                  <ul className="space-y-2 text-sm">
                    {post.headings.map((heading, index) => (
                      <li key={index}>
                        <a
                          href={`#section-${index}`}
                          className="text-gray-500 hover:text-[#00568c] transition-colors"
                        >
                          {heading}
                        </a>
                      </li>
                    ))}
                    {post.faqs.length > 0 && (
                      <li>
                        <a
                          href="#faqs"
                          className="text-gray-500 hover:text-[#00568c] transition-colors"
                        >
                          FAQs
                        </a>
                      </li>
                    )}
                  </ul>
                </nav>
              </div>
            </aside>
          )}

          {/* Article Content */}
          <article className={`${post.headings.length > 0 ? 'lg:col-span-3' : 'lg:col-span-4 max-w-4xl'}`}>
            {/* Mobile TOC */}
            {post.headings.length > 0 && (
              <div className="lg:hidden bg-gray-50 rounded-xl p-6 mb-8">
                <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-3">
                  Table of Contents
                </h2>
                <ul className="space-y-2 text-sm">
                  {post.headings.map((heading, index) => (
                    <li key={index}>
                      <a
                        href={`#section-${index}`}
                        className="text-[#00568c] hover:text-[#00568c]/80"
                      >
                        {heading}
                      </a>
                    </li>
                  ))}
                </ul>
              </div>
            )}

            {/* Featured Image */}
            <div className="relative w-full aspect-video rounded-xl overflow-hidden mb-8">
              <Image
                src={post.image || FALLBACK_IMAGE}
                alt={post.imageAlt || FALLBACK_IMAGE_ALT}
                fill
                className="object-cover"
                priority
                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 75vw, 900px"
              />
            </div>

            {/* Content Sections */}
            <div className="prose max-w-none">
              {post.content.map((section, index) => (
                <section
                  key={index}
                  id={`section-${index}`}
                  className="mb-8"
                  dangerouslySetInnerHTML={{ __html: section }}
                />
              ))}
            </div>

            {/* FAQs */}
            {post.faqs.length > 0 && (
              <section id="faqs" className="mt-12 pt-8 border-t border-gray-200">
                <h2 className="text-2xl font-bold text-gray-900 mb-6">
                  Frequently Asked Questions
                </h2>
                <div className="space-y-6">
                  {post.faqs.map((faq, index) => (
                    <div key={index} className="bg-gray-50 rounded-xl p-6">
                      <h3 className="text-lg font-semibold text-gray-900 mb-3">
                        {faq.question}
                      </h3>
                      <p className="text-gray-600 leading-relaxed">{faq.answer}</p>
                    </div>
                  ))}
                </div>
              </section>
            )}

            {/* Author Bio */}
            <section className="mt-12 border-t border-gray-200 pt-8">
              <div className="flex items-start gap-4">
                <div className="w-16 h-16 rounded-full overflow-hidden flex-shrink-0">
                  <Image
                    src="/images/pool/kyle-bowman.webp"
                    alt="Kyle Bowman"
                    width={128}
                    height={128}
                    className="w-full h-full object-cover"
                  />
                </div>
                <div>
                  <Link href="/about/kyle-bowman/" className="text-lg font-semibold text-gray-900 hover:text-[#00568c] transition-colors">
                    Kyle Bowman
                  </Link>
                  <p className="text-sm text-[#00568c] font-medium mb-2">Founder of PoolLogic · CPO-Certified</p>
                  <p className="text-gray-600 text-sm leading-relaxed">
                    Kyle is the Founder of PoolLogic Pool Service and a Certified Pool Operator (CPO) with deep expertise in pool service, water chemistry, and pool equipment repair. He oversees every aspect of PoolLogic&apos;s operations to ensure San Diego pool owners receive expert-level care.
                  </p>
                </div>
              </div>
            </section>

            {/* CTA */}
            <section className="mt-12 bg-[#88d2c7]/10 rounded-2xl p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">
                Need Professional Pool Service?
              </h2>
              <p className="text-gray-600 mb-6">
                {company.name} provides expert pool cleaning, maintenance, and repair services throughout San Diego County.
              </p>
              <DualCTA quoteText="Get a Free Quote" variant="compact" />
            </section>
          </article>
        </div>
      </div>

      {/* Related Articles */}
      {relatedPosts.length > 0 && (
        <section className="py-12 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">Related Articles</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedPosts.map((related) => (
                <Link
                  key={related.slug}
                  href={`/resources/${related.slug}/`}
                  className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow group"
                >
                  <div className="relative w-full aspect-video">
                    <Image
                      src={related.image || FALLBACK_IMAGE}
                      alt={related.imageAlt || related.title}
                      fill
                      className="object-cover group-hover:scale-105 transition-transform duration-300"
                      sizes="(max-width: 768px) 100vw, 33vw"
                    />
                  </div>
                  <div className="p-6">
                    <span className="text-xs font-medium text-[#00568c] uppercase tracking-wide">
                      {related.categoryName}
                    </span>
                    <h3 className="text-lg font-semibold text-gray-900 mt-2 mb-2 group-hover:text-[#00568c] transition-colors">
                      {related.title}
                    </h3>
                    <p className="text-gray-600 text-sm line-clamp-2">{related.excerpt}</p>
                    <div className="mt-3 text-sm text-gray-500">{related.readTime}</div>
                  </div>
                </Link>
              ))}
            </div>
          </div>
        </section>
      )}
    </>
  );
}
